...
def haiguitang_init_state(state):
...
o=0
#游戏控制，开局或者重启函数
def haiguitang_fn_start_or_restart(haiguitang_user_chatbot,state,stats):
    # 初始化state
    global o
    o=1
    #state = haiguitang_init_state(state)  #创建全局变量
    ...
    #print(state)
    return haiguitang_user_chatbot,state,stats


def main(user_chatbot,state,chat_input,stats) -> None:
    """group chat"""
    global o
        ...
        if state["step"] == 0:
            ...
            with msghub(agents, announcement=hint) as hub:
                ...
                #while True:
                    ...
                    #rnd += 1
                        #x = host_agent(x)
    return  haiguitang_user_chatbot,state,haiguitang_chat_input,stats
...
#游戏控制，开局或者重启函数
def fn_start_or_restart(user_chatbot,state,stats):
    # 初始化state
    #state = init_state(state)  #创建全局变量
    state["in_game"] = True
    ...
    return state, user_chatbot,f"你的词语是 `{state['player_words'][0]}`，请进行发言", "", "", "", "", "未选择", "", "", "", "Source/four.jpg",stats
...
def fn_speek(state, prompt,user_chatbot,stats):
    system_text = ""
...
def fn_vote(state, vote_prompt,user_chatbot,stats):
...
j=0
msg = None
def gomoku_send_message(chatbot, input, _state,stats):
        #_state=gomoku_init_user(_state)
        global j 
        global msg
        if j == 0:
            if input == '开始':
...
y=0
def send_message(chatbot, chatsys, user_input, _state,stats):
        # 然后使用 message 变量来发送消息
        # 将发送的消息添加到聊天历史
        global y
        #_state = init_game(_state)
...
with demo:
    ...
    preview_send_button1.click(
        gomoku_send_message,
        inputs=[user_chatbot1, preview_chat_input1, state, stats],
    ...
    user_chat_input.submit(
        send_message,
        inputs=[user_chatbot, user_chatsys, user_chat_input, state,stats],
    ...
    start_btn.click(fn=fn_start_or_restart,
                    inputs=[wodi_user_chatbot,state,stats],
                    outputs=[state,
                             wodi_user_chatbot,
                             system_text,
    ...
    speek_btn.click(fn=fn_speek,
                    inputs=[state, player1_text,wodi_user_chatbot,stats],
                    outputs=[state, wodi_user_chatbot, system_text, player2_text, player3_text, player4_text,stats])
    vote_btn.click(fn=fn_vote,
                   inputs=[state, player1_vote,wodi_user_chatbot,stats],
                   outputs=[state,
                            wodi_user_chatbot,
                            system_text,
                            player1_text, player2_text, player3_text, player4_text,
    ...
    start_btn_haituitang.click(haiguitang_fn_start_or_restart,inputs=[haiguitang_user_chatbot,state,stats],outputs=[haiguitang_user_chatbot,state,stats])
    haiguitang_send_button.click(main,inputs=[haiguitang_user_chatbot,state,haiguitang_chat_input,stats],outputs=[haiguitang_user_chatbot,state,haiguitang_chat_input,stats])
    
    #demo.load(haiguitang_init_state,inputs=[state],outputs=[state])
