...
Please break down the case description into 3-5 small sub-scenes, describing the content of the text
You should respond in the json format, which can be loaded by json in Python(pleas don't show 'json'):
{
...
Please ensure that your response follows the specified format and includes the necessary fields.
{
    ...
    "scripts": ['script1 is The novelist sits at his desk, with a distorted face and a painful expression, showing the struggle before death; his hand is gripping a pen, and the window behind him is closed and undamaged, emphasizing that the events occurring in the room were not caused by external intrusion.', 'script2 is The unfinished manuscript is laid out, with the pen resting on the paper and the ink still wet, suggesting that the novelist was writing a chapter closely related to his last work before his death','script3 is A crumpled photo was abandoned in a corner or on the ground, clearly showing a photo of the novelist with a strange woman. The identity of this mysterious woman has led to more speculation about the novelist's life experience and cause of death.']
}
# forbitten
...
def main(user_chatbot,chat_input,stats,uid) -> None:
    ...
    if state[uid]['o'] == 0:
        ...
        if state[uid]["step"] == 0:
            ...
            data_url = generate_data_url(wav_file_path)
            audiourl= f'<audio src="{data_url}"/>'
            ...
            ##image_result = text2image(input=response)
            #user_chatbot.append((None,f"{image_results[0]}"))
            image_src = covert_image_to_base64('ci_workspace/output_image_20240412104138.png')
            ...
            with msghub(participants=[state[uid]['haiguitang_host_agent'],state[uid]['haiguitang_assi_agent']],announcement=state[uid]['hint']):
                ...
                if "还原" in chat_input:
                    ...
                    x = Msg(name=f'{x.name}',content=f"这是来自{x.name}的提问：{json_dict_str}，请回答问题，每次回答“是”、“否”、或“与此无关”后，还要告知用户剩余提问次数。但不需要引导用户问题",role="assistant")
                    x = state[uid]['haiguitang_host_agent'](x)
                    user_chatbot.append((None,f"主持人的回复：{x.content}"))
                    state[uid]['history'].append([f'host:{x.content}'])
                    yield {
        ...
        if state[uid]['j'] == 0:
            ...
            with msghub(participants=[state[uid]['black'], state[uid]['white'], state[uid]['board']]):
                ...
                # remind the player to make a move
                            ...
                            state[uid]['gomoku_history'].append(input)
                            yield {
                                            user_chatbot1: chatbot,
                                            preview_chat_input1: '',
                                    }
                            ...
                            if is_valid_input(input):
                                ...
                                if state[uid]['board'].game_end:
                                    ...
                                    msg = Msg(name="system", content=f"{state[uid]['board_msg'].content},已经在棋盘下过的棋子有{str(state[uid]['gomoku_history'])}", role="aissiant")
                                    state[uid]['white_msg'] = state[uid]['white'](msg)
                                    print(msg.content)
                                    chatbot.append((None, f"Bob出手是{state[uid]['white_msg'].content}"))
                                    state[uid]['gomoku_history'].append(state[uid]['white_msg'].content)
                                    yield {
                                                    user_chatbot1: chatbot,
                                                    preview_chat_input1: '',
                                            }
...
demo.launch(max_threads=400,share=True)
