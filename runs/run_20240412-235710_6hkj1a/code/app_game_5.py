...
HINT_PROMPT = """
check-role:
1.水平连线：在任何行中，连续三个相同的非空字符。表达为："XXX" 或 "OOO" 在检查时，可以通过取字符串子片段并比较来实现。
2.垂直连线：在任何列中，连续三个相同的非空字符。每个垂直连线也可以被视为字符串："XXX" 或 "OOO" 检查时，可以按列提取字符并构造字符串。
3.主对角线连线（从左上到右下）：在对角线上，连续三个相同的非空字符。表达为："XXX" 或 "OOO" 遍历对角线元素，构造字符串进行检查。
4.次对角线连线（从右上到左下）：在对角线上，连续三个相同的非空字符。表达为："XXX" 或 "OOO" 同样，遍历对角线元素，构造字符串进行检查。
strategy:
