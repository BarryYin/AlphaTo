...
#游戏控制，开局或者重启函数
def haiguitang_fn_start_or_restart(haiguitang_user_chatbot,state):
    # 初始化state
    ...
    #print(state)
    return haiguitang_user_chatbot,state

def main(user_chatbot,state,chat_input) -> None:
    """group chat"""
    state = haiguitang_init_state(state) 
    ...
    if state["haiguitang_in_game"] == False:
        ...
        if state["step"] == 0:
            ...
            with msghub(agents, announcement=hint) as hub:
                ...
                #while True:
                    ...
                    #rnd += 1
                        #x = host_agent(x)
    return  haiguitang_user_chatbot,state,haiguitang_chat_input
...
j=0
msg = None
def gomoku_send_message(chatbot, input, _state,uid):
...
with demo:
    ...
    preview_send_button1.click(
        gomoku_send_message,
        inputs=[user_chatbot1, preview_chat_input1, state,uid],
    ...
    start_btn_haituitang.click(haiguitang_fn_start_or_restart,inputs=[haiguitang_user_chatbot,state],outputs=[haiguitang_user_chatbot,state])
    haiguitang_send_button.click(main,inputs=[haiguitang_user_chatbot,state,haiguitang_chat_input],outputs=[haiguitang_user_chatbot,state,haiguitang_chat_input])
