...
3. You should think carefully about your strategy and moves, considering both your and your opponent's subsequent moves.
4. Make sure you don't place your piece on a spot that has already been occupied. like spot [5,6] is already occupied, you can't place your piece on it.
5. Only an unbroken line of five same pieces will win the game. For example, "xxxoxx" won't be considered a win.
6. Note the unbroken line can be formed in any direction: horizontal, vertical, or diagonal.

strategy:
1. Defense comes first. Determine whether the opponent's previous moves have formed three connected lines, and whether there are three pieces closely attached in the vertical, horizontal, left diagonal, and right diagonal directions on the board, in the form of OOO. If so, place a piece next to the edge of the opponent's piece, such as OOOX.
...
workflow:
step1. based on the opponent's pieces, determine whether the opponent has three pieces that are closely aligned, and pick out the location of the piece. 
step2. Next, place a piece next to the opponent's three pieces that are closely aligned to prevent the opponent's four pieces from forming a line. 
step3. If the opponent's pieces do not have three closely aligned pieces, check whether your own pieces have three pieces that are closely aligned. 
step4. If your own pieces do not have three pieces that are closely aligned, find the piece that has been placed next to the next piece to facilitate the formation of a line of your own pieces.
...
def gomoku_send_message(chatbot, input, stats,uid):
        ...
        if state[uid]['j'] == 0:
            if input == '开始':
                ...
                #global i
                msg = None
                state[uid]['board_msg'] = state[uid]['board'](msg)
                chatbot.append((None, f"当前的棋局是{state[uid]['board_msg'].content}"))
                yield {
                                        user_chatbot1: chatbot,
                                        preview_chat_input1: '',
                                }
            ...
            with msghub(participants=[state[uid]['black'], state[uid]['white'], state[uid]['board']]):
                if not state[uid]['board'].game_end and state[uid]['i'] < MAX_STEPS:
                ...
                # remind the player to make a move
                            ...
                            state[uid]['gomoku_White_history'].append(input)
                            yield {
                                            user_chatbot1: chatbot,
                                            preview_chat_input1: '',
                                    }
                            ...
                            if is_valid_input(input):
                                ...
                                time.sleep(1)
                                if state[uid]['board'].game_end:
                                    pass
                                else:
                                    msg = Msg(name="system", content=f"{state[uid]['board_msg'].content},对手已经在棋盘下过的棋子有{str(state[uid]['gomoku_White_history'])},你已经在棋盘下过的棋子有{str(state[uid]['gomoku_Black_history'])}", role="aissiant")
                                    state[uid]['white_msg'] = state[uid]['white'](msg)
                                    print(msg.content)
                                    chatbot.append((None, f"Bob出手是{state[uid]['white_msg'].content}"))
                                    state[uid]['gomoku_Black_history'].append(state[uid]['white_msg'].content)
                                    yield {
                                                    user_chatbot1: chatbot,
                                                    preview_chat_input1: '',
                                            }
                                    state[uid]['board_msg'] = state[uid]['board'](state[uid]['white_msg'])
                                    #print(msg.content)
                                    chatbot.append((None, f"当前的棋局是{state[uid]['board_msg'].content}"))
                                    yield {
                                            user_chatbot1: chatbot,
...
#y=0
def send_message(chatbot, chatsys, user_input, stats,uid):
        # 然后使用 message 变量来发送消息
        # 将发送的消息添加到聊天历史
        #global y
        #state = init_game(state)
       
        #state1.setdefault('y', 0)
        #print(state1['y'])

        #state[uid]['host_agent'] = agents[0]
        #state[uid]['judge_agent'] = agents[1]
        #state[uid]['judge_AI_agent'] = agents[2]
        #state[uid]['parti_agent'] = agents[3]
        #host_agent = state[uid]['host_agent']
        #judge_agent = state[uid]['judge_agent']
        #judge_AI_agent = state[uid]['judge_AI_agent']
        #parti_agent = state[uid]['parti_agent']
        ...
        if state[uid]['y'] ==0:
            if('开始' in user_input):
                        ...
                        state[uid]['host_msg'] = state[uid]['host_agent'](msg)
                        print(state[uid]['host_msg'])
                        chatsys.append((f"主持人：本轮的关键字是：{state[uid]['host_msg'].content}", None))
                        yield {
                            user_chatbot: chatbot,
                            user_chatsys: chatsys,
                        }
                        #global pre_host_key
                        state[uid]['pre_host_key'] = state[uid]['host_msg'].content
                        state[uid]['y'] =state[uid]['y'] + 1
                        print(state[uid]['y'])
            else:
                #print('请输入：开始')
            ...
            if '开始' in user_input or  '继续' in user_input:
                        ...
                        state[uid]['host_msg'] = state[uid]['host_agent'](msg)
                        chatsys.append((f"主持人：本轮的谜语是：{state[uid]['host_msg'].content}，请输入你的答案", None))
                        yield {
                            user_chatbot: chatbot,
                            user_chatsys: chatsys,
                        }
                        ...
                        state[uid]['y'] =state[uid]['y'] +1
                            ...
                            judge_content = f"主持人的谜语是{state[uid]['pre_host_key']}，用户的谜底是{user_input}，请确认用户的得分，是否赢得了比赛"
                            state[uid]['judge_msg'] = state[uid]['judge_agent'](Msg(name='judge', content=judge_content))
                            chatsys.append((None, f"评审官：{state[uid]['judge_msg'].content}"))
                            yield {
                                user_chatbot: chatbot,
                                user_chatsys: chatsys,
                            }
                            state[uid]['y'] +=1
                            time.sleep(1)
                            if '7' not in state[uid]['judge_msg'].content:
                                ...
                                time.sleep(1)
                                parti_content = f"主持人的谜语是{state[uid]['pre_host_key']}"
                                state[uid]['parti_msg'] = state[uid]['parti_agent'](Msg(name='parti', content=parti_content))
                                chatbot.append((None, f"AI-Agent答题：{state[uid]['parti_msg'].content} 我是AI界的光头强"))
                                yield {
                                        user_chatbot: chatbot,
                                        user_chatsys: chatsys,
                                    }
                                judge_AI_content = f"主持人的谜语是{state[uid]['pre_host_key']}，AI-Agent的谜底是{state[uid]['parti_msg'].content}，请确认AI-Agent的得分，是否赢得了比赛"
                                state[uid]['judge_AI_msg'] = state[uid]['judge_AI_agent'](Msg(name='judge', content=judge_AI_content))
                                chatsys.append((None, f"评审官：{state[uid]['judge_AI_msg'].content}"))
                                yield {
                                        user_chatbot: chatbot,
                                        user_chatsys: chatsys,
                                    }
                                if '7' not in state[uid]['judge_AI_msg'].content:
            ...
            state[uid]['judge_msg'] = state[uid]['judge_AI_agent'](Msg(name='judge', content='本轮游戏结束，请将选手得分score重新初始化为5'))
...
def updateimg(uid):
    ...
    if state[uid]['j']==0:
        ...
        img = f"assets/current_boarde_{uid}.png"
    return img
...
with demo:
    with gr.Row():
        ...
        createuid =gr.Button(value="创建UUID",visible=True)
    ...
    with game_tabs:
        ...
        with main_tab1:
            ...
            # 开始游戏按钮
            with gr.Row():
              start_btn = gr.Button(value="开始游戏")
            with gr.Row():
            # 一张图片
                with gr.Column(min_width=270):
                        wodi_user_chatbot = gr.Chatbot(
                            height=500,
                            value=[['您好，欢迎来到谁是卧底游戏，如果你准备好了，请点击上方「开始」按钮', None]],
                            elem_classes="app-chatbot",
                ...
                with gr.Column():
                    system_text = gr.Textbox(label="系统信息展示框", value="点击开始游戏 进行游戏")
                    with gr.Row():
                        with gr.Column():
    ...
    preview_send_button1.click(
        gomoku_send_message,
        inputs=[user_chatbot1, preview_chat_input1,  stats,uidinput],
    ...
    user_chat_input.submit(
        send_message,
        inputs=[user_chatbot, user_chatsys, user_chat_input, stats,uidinput],
    ...
    createuid.click(fn=generate_uid, inputs=[], outputs=[uidinput])
    start_btn.click(fn=fn_start_or_restart,
                    inputs=[wodi_user_chatbot,stats,uidinput],
                    outputs=[
                             wodi_user_chatbot,
                             system_text,
    ...
    start_btn_haituitang.click(haiguitang_fn_start_or_restart,inputs=[haiguitang_user_chatbot,stats,uidinput],outputs=[haiguitang_user_chatbot,stats])
    haiguitang_send_button.click(main,inputs=[haiguitang_user_chatbot,haiguitang_chat_input,stats,uidinput],outputs=[haiguitang_user_chatbot,haiguitang_chat_input,stats])
    
    #demo.load(refresh_game,inputs=[stats],outputs=[stats])
