...
def main(user_chatbot,chat_input,stats,uid) -> None:
    ...
    if state[uid]['o'] == 0:
        ...
        if state[uid]["step"] == 0:
            ...
            #speak_list = []
            #i = 0
            with msghub(state[uid]['haiguitang_host_agent'],state[uid]['haiguitang_assi_agent'], announcement=state[uid]['hint']) as hub:
                x = Msg(name="haiguitang_user",content=chat_input,role="user")
                user_chatbot.append((f"用户输入：{chat_input}",None))
                yield {
                    haiguitang_user_chatbot: user_chatbot,
                ...
                if "还原" in chat_input:
                    ...
                    user_chatbot.append((None,f"主持人的回复：{state[uid]['haiguitang_host_agent'].content}"))
                    yield {
                    haiguitang_user_chatbot: user_chatbot,
                    haiguitang_chat_input: '',
                                    }
                    state[uid]['history'].append([f"host:{state[uid]['haiguitang_host_agent'].content}"])
                    #break
                else:
                    x = state[uid]['haiguitang_host_agent'](x)
                    user_chatbot.append((None,f"主持人的回复：{state[uid]['haiguitang_host_agent'].content}"))
                    ...
                    tate[uid]['history'].append([f'host:{x.content}'])
                    yield {
                    haiguitang_user_chatbot: user_chatbot,
                    haiguitang_chat_input: '',
                                    }
                ...
                # remind the player to make a move
                            ...
                            if is_valid_input(input):
                                ...
                                if state[uid]['board'].game_end:
                                    ...
                                    state[uid]['white_msg'] = state[uid]['white'](state[uid]['board_msg'])
...
with demo:
    ...
    with gr.Row():
        ...
        createuid =gr.Button(value="创建UUID")
