...
#游戏控制，开局或者重启函数
def fn_start_or_restart(user_chatbot):
    # 初始化state
    state = init_state()  #通过这个函数，获取唯一的seesion, 可加一个uid，作为用户标识
    state["in_game"] = True
...
with demo:
    ...
    demo.load(init_state,inputs=[], outputs=[])
